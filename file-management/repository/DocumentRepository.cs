    ...
    public class DocumentRepository : IDocumentRepository
    {
        ...
        */
            ...
            int pageSize = 25)
        {
            ...
            // Filter
            if (!string.IsNullOrWhiteSpace(queryByName) && queryByStartActivedDate != null && queryByDocumentCategoryId != null)
            {
            ...
            else if (!string.IsNullOrWhiteSpace(queryByName) && queryByStartActivedDate != null)
            {
                query = queryByEndActivedDate switch
                {
                    null => query.Where(q =>
            ...
            else if (queryByStartActivedDate != null && queryByDocumentCategoryId != null)
            {
                query = queryByEndActivedDate switch
                {
                    null => query.Where(q =>
            ...
            else if (!string.IsNullOrWhiteSpace(queryByName) && queryByDocumentCategoryId != null)
            {
                query = query.Where(q =>
                    q.Name.ToLower().Contains(queryByName.ToLower()) &&
                    q.DocumentCategoryId == queryByDocumentCategoryId);
            }
            else if (!string.IsNullOrWhiteSpace(queryByName))
            {
                query = query.Where(q => q.Name.ToLower().Contains(queryByName.ToLower()));
            }
            else if (queryByStartActivedDate != null)
            {
                if (queryByEndActivedDate == null)
                    query = query.Where(q => q.ActivedDate.Date == queryByStartActivedDate.Value.Date);
                else
                    query = query.Where(q =>
                                             q.ActivedDate.Date >= queryByStartActivedDate.Value.Date &&
                                             q.ActivedDate.Date <= queryByEndActivedDate.Value.Date);
            }
            else if (queryByDocumentCategoryId != null)
            {
                query = query.Where(q => q.DocumentCategoryId == queryByDocumentCategoryId);
            }
            else
            {
